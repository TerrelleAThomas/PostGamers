...
import com.example.postgamers.Model.RestPost;
...
public class PostService {
    ...
    private Post getPost(DocumentSnapshot doc) throws ExecutionException, InterruptedException {
        ...
        User user = userService.getUser((DocumentReference) doc.get("gamerId") );

        ArrayList<Genre> categories = new ArrayList<>();
        ArrayList<DocumentReference> refs = (ArrayList<DocumentReference>) doc.get("genreId");
        for(DocumentReference ref : refs)
        {
            ApiFuture<DocumentSnapshot> catQuery = ref.get();
            DocumentSnapshot catDoc = catQuery.get();
            Genre genre = catDoc.toObject(Genre.class);
    ...
    public ArrayList<Post> getPostsByGenre(String category) throws ExecutionException, InterruptedException {
            ...
            if(list.contains(key))
            {
                switch(key)
                {
                    ...
                    case "genreId":
                        ...
                        {
                            DocumentReference catRef = db.collection("Genre").document(cat);
                            if(catRef != null)
                                catRefs.add(catRef);
                        }
                        if(catRefs.size() > 0)
